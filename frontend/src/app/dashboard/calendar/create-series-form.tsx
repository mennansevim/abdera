...
export function CreateSeriesForm({ onCreated, initialDate, initialDay, initialTime }: { onCreated?: () => void; initialDate?: string; initialDay?: string; initialTime?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="text-micro text-[var(--brand-strong)]">Akıllı zamanlayıcı</p><h2 className="mt-1 text-title">Yeni ders serisi</h2><p className="text-meta mt-1">Öğrenciyi seç; sistem öğretmen ve öğrenci takvimini birlikte tarasın.</p></div>
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--success-soft)] px-2.5 py-1.5 text-[.65rem] font-bold text-[var(--success-strong)]"><Icon name="check" className="h-3.5 w-3.5" /> Çakışma kontrolü açık</span>
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">1 · Öğrenci<select value={studentId} onChange={(event) => { setStudentId(event.target.value); setEnrollmentId(""); setSelectedSlot(null); }} required className="field text-sm"><option value="">Öğrenci seç</option>{students?.map((student) => <option key={student.id} value={student.id}>{student.firstName} {student.lastName}</option>)}</select></label>
        <label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">2 · Ders ve öğretmen<select value={enrollmentId} onChange={(event) => { setEnrollmentId(event.target.value); setSelectedSlot(null); }} required disabled={!studentId} className="field text-sm"><option value="">Aktif kayıt seç</option>{activeEnrollments.map((item) => { const itemTeacher = teachers?.find((teacherItem) => teacherItem.id === item.teacherId); const itemInstrument = instruments?.find((instrumentItem) => instrumentItem.id === item.instrumentId); return <option key={item.id} value={item.id}>{itemTeacher ? `${itemTeacher.firstName} ${itemTeacher.lastName}` : "Öğretmen"} · {itemInstrument?.name ?? "Ders"}</option>; })}</select></label>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[12rem_14rem_1fr]">
        <label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">Ders süresi<select value={durationMinutes} onChange={(event) => { setDurationMinutes(Number(event.target.value)); setSelectedSlot(null); }} className="field text-sm"><option value={30}>30 dakika</option><option value={45}>45 dakika</option><option value={60}>60 dakika</option></select></label>
        <label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">Başlangıç tarihi<input type="date" value={effectiveFrom} onChange={(event) => { setEffectiveFrom(event.target.value); setSelectedSlot(null); }} required className="field text-sm" /></label>
      ...
      <div className="flex items-start gap-2.5 rounded-xl border border-[var(--line)] bg-[var(--surface-muted)] px-3 py-2.5 text-[.68rem] leading-relaxed text-[var(--muted)]">
        <Icon name="calendar" className="mt-0.5 h-4 w-4 shrink-0 text-[var(--brand)]" />
        <p>Seçilen gün ve saat <strong className="text-[var(--foreground)]">her hafta aynı şekilde</strong> tekrarlanır. Bir öğrenci haftada en fazla <strong className="text-[var(--foreground)]">4 düzenli ders</strong> alabilir.</p>
      </div>

      {enrollment && !showManual && (
        <section className="rounded-2xl border border-[var(--line)] bg-[#fffaf4] p-3 sm:p-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-bold">Önerilen saatler</p><p className="text-[.65rem] text-[var(--muted)]">Öğretmenin müsaitliği ve 8 haftalık çakışmalar tarandı.</p></div>{availabilityLoading && <span className="text-[.65rem] text-[var(--muted)]">Takvim taranıyor…</span>}</div>
      ...
      {showManual && <div className="grid gap-3 rounded-2xl border border-[var(--line)] bg-[var(--surface-muted)] p-4 sm:grid-cols-2"><label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">Gün<select value={manualDay} onChange={(event) => setManualDay(event.target.value)} className="field text-sm">{DAY_KEYS.slice(1).concat(DAY_KEYS[0]!).map((day) => <option key={day} value={day}>{DAY_NAMES_TR[day]}</option>)}</select></label><label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">Saat<input type="time" value={manualTime} onChange={(event) => setManualTime(event.target.value)} className="field text-sm" /></label></div>}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--line)] pt-4"><button type="button" onClick={() => setShowManual((value) => !value)} className="pressable min-h-10 rounded-xl px-3 text-xs font-bold text-[var(--muted)] hover:bg-[var(--surface-muted)]">{showManual ? "Akıllı önerilere dön" : "Özel saat belirle"}</button><button type="submit" disabled={createSeries.isPending || !enrollmentId || (!showManual && !selectedSlot && !suggestions.length)} className="pressable min-h-11 rounded-xl bg-[var(--brand)] px-5 text-sm font-bold text-white shadow-[0_8px_18px_rgba(217,102,42,.2)] disabled:opacity-50">{createSeries.isPending ? "Takvime yerleştiriliyor…" : "Seriyi takvime yerleştir"}</button></div>
    </form>
  );
}
