...
export function MakeupScheduler({ onPlaced, onCancel }: { onPlaced?: (summary: string) => void; onCancel?: () => void }) {
  ...
  return <section className="space-y-4">
    ...
    <div className="grid gap-3 lg:grid-cols-3">
      <label className="form-label">Öğrenci<select value={activeStudentId} onChange={(event) => { setStudentId(event.target.value); setEnrollmentId(""); setCreditId(""); setSelectedSlot(null); }} className="field text-sm"><option value="">Telafi hakkı olan öğrenciyi seç</option>{eligibleStudents.map((student) => <option key={student.id} value={student.id}>{student.firstName} {student.lastName}</option>)}</select>{creditQueries.some((query) => query.isLoading) && <span className="block text-[.75rem] font-medium">Telafi hakları kontrol ediliyor…</span>}{!creditQueries.some((query) => query.isLoading) && !eligibleStudents.length && <span className="block text-[.75rem] font-medium text-[var(--warning-strong)]">Kullanılabilir telafi hakkı olan öğrenci yok.</span>}</label>
