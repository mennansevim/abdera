...
export function MakeupScheduler({ onPlaced }: { onPlaced?: () => void }) {
  ...
  async function place() {
    ...
    try {
      ...
      setMessage({ tone: "success", text: `Telafi dersi ${formatSlot(slot).day} ${formatSlot(slot).time} saatine yerleştirildi.` });
  ...
  return <section className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-micro text-[var(--brand-strong)]">Telafi asistanı</p><h2 className="mt-1 text-title">Uygun slot bul ve yerleştir</h2><p className="text-meta mt-1">Öğretmen ve öğrenci programındaki ortak boşlukları tek tıkla kullan.</p></div><span className="inline-flex items-center gap-1.5 rounded-full bg-[#e6dcf6] px-2.5 py-1.5 text-[.65rem] font-bold text-[#4b3777]"><Icon name="activity" className="h-3.5 w-3.5" /> 21 gün taranır</span></div>
    <div className="grid gap-3 lg:grid-cols-3">
      <label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">Öğrenci<select value={activeStudentId} onChange={(event) => { setStudentId(event.target.value); setEnrollmentId(""); setCreditId(""); setSelectedSlot(null); }} className="field text-sm"><option value="">Telafi hakkı olan öğrenciyi seç</option>{eligibleStudents.map((student) => <option key={student.id} value={student.id}>{student.firstName} {student.lastName}</option>)}</select>{creditQueries.some((query) => query.isLoading) && <span className="block text-[.62rem] font-medium">Telafi hakları kontrol ediliyor…</span>}{!creditQueries.some((query) => query.isLoading) && !eligibleStudents.length && <span className="block text-[.62rem] font-medium text-[var(--warning-strong)]">Kullanılabilir telafi hakkı olan öğrenci yok.</span>}</label>
      <label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">Ders<select value={enrollment?.id ?? ""} onChange={(event) => { setEnrollmentId(event.target.value); setSelectedSlot(null); }} disabled={!activeStudentId} className="field text-sm"><option value="">Aktif kayıt seç</option>{activeEnrollments.map((item) => { const teacher = teachers?.find((row) => row.id === item.teacherId); const instrument = instruments?.find((row) => row.id === item.instrumentId); return <option key={item.id} value={item.id}>{instrument?.name ?? "Ders"} · {teacher?.firstName} {teacher?.lastName}</option>; })}</select></label>
      <label className="space-y-1.5 text-xs font-bold text-[var(--muted)]">Telafi hakkı<select value={activeCreditId} onChange={(event) => setCreditId(event.target.value)} disabled={!activeStudentId} className="field text-sm"><option value="">Telafi hakkı seç</option>{activeCredits.map((credit) => <option key={credit.id} value={credit.id}>Son kullanım {new Date(credit.expiresAt).toLocaleDateString("tr-TR")}</option>)}</select></label>
    </div>
    {enrollment && activeCreditId && <div className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-bold">En uygun boşluklar</p><label className="inline-flex items-center gap-2 text-[.65rem] font-bold text-[var(--muted)]">Süre<select value={durationMinutes} onChange={(event) => { setDurationMinutes(Number(event.target.value)); setSelectedSlot(null); }} className="rounded-lg border border-[var(--line)] bg-white px-2 py-1.5"><option value={30}>30 dk</option><option value={45}>45 dk</option><option value={60}>60 dk</option></select></label></div>{isLoading ? <div className="grid gap-2 sm:grid-cols-3">{[1,2,3].map((item) => <span key={item} className="skeleton h-16 rounded-xl" />)}</div> : suggestions.length ? <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{suggestions.map((slot, index) => { const active = (selectedSlot ?? suggestions[0])?.start.getTime() === slot.start.getTime(); const formatted = formatSlot(slot); return <button type="button" key={slot.start.toISOString()} onClick={() => setSelectedSlot(slot)} className={`pressable flex min-h-16 items-center gap-3 rounded-xl border p-3 text-left ${active ? "border-[#6d559b] bg-[#eee8f8]" : "border-[var(--line)] bg-white hover:border-[#6d559b]"}`}><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg text-xs font-bold ${active ? "bg-[#6d559b] text-white" : "bg-[var(--surface-muted)] text-[var(--muted)]"}`}>{index + 1}</span><span><span className="block text-xs font-bold capitalize">{formatted.day}</span><span className="mt-1 block text-[.65rem] font-semibold text-[var(--muted)]">{formatted.time}</span></span></button>; })}</div> : <p className="rounded-xl bg-[var(--danger-soft)] px-3 py-3 text-xs font-semibold text-[var(--danger-strong)]">Önümüzdeki 21 günde uygun ortak boşluk bulunamadı.</p>}</div>}
    ...
    <div className="flex justify-end border-t border-[var(--line)] pt-4"><button type="button" onClick={place} disabled={useCredit.isPending || !activeCreditId || !enrollment || (!selectedSlot && !suggestions.length)} className="pressable min-h-11 rounded-xl bg-[#6d559b] px-5 text-sm font-bold text-white shadow-[0_8px_18px_rgba(75,55,119,.18)] disabled:opacity-50">{useCredit.isPending ? "Yerleştiriliyor…" : "Seçilen slota yerleştir"}</button></div>
  </section>;
}
