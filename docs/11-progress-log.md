...
- **Küçük, bilerek ertelenen bulgu:** Aidatlar sayfasındaki fiyat listesi bölümü kullanıcının istediği "Kurs Bedeli" adı yerine hâlâ "Fiyat Listeleri" başlığını taşıyor - işlevsel olarak istenen şeyi yapıyor (enstrüman+süre bazlı birim fiyat), yalnızca isimlendirme farkı. Küçük bir takip olarak flag'lendi, bu oturumda kapsam dışı tutuldu.
- Ortamla ilgili not (kod hatası değil): `abdera-web-api-1` bu oturumda bir kez host RAM baskısıyla SIGSEGV ile çöktü, yeniden başlatılınca sorunsuz devam etti - bu davranış daha önce de görülmüş ve kayda geçirilmiş (bkz. aşağıdaki "UI takip" bölümü).

- [ ] Faz 1-2 doğrulaması + migration düzeltmesi + yeni testler commit edilecek (`abdera-commit` skill), push için ayrıca kullanıcı onayı istenecek.
- [ ] Sonraki adım: **Faz 3 — mesaj otomasyonu** (kalıcı hatırlatma-süresi ayarı, 3'lü RSVP `AttendingLate`, `AutomationSettings` bileşeninin gerçek API'ye bağlanması). CLAUDE.md'nin Hangfire/Quartz yasağı nedeniyle mevcut `INotificationScheduler`/`NotificationDispatcher` (`BackgroundService`) mimarisi korunacak, `docs/15-product-phases.md`'deki Hangfire notu yanlış - düzeltilecek.
- [ ] Faz 4 (sağlık kontrolü + yedekleme + e-posta alarmı) henüz planlanmadı - SMTP sağlayıcısı, yedekleme hedefi/saklama süresi gibi kullanıcı kararları gerekiyor.
