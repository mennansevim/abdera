...
| M9 | Sabit aylık gider | Kullanıcı isteği: "her ay girilmesin, aylık bu kadar elektrik su gelir ortalama diye gidilsin; kira girildiği zaman tüm yıl aynı gitsin; güncellendiğinde eski veriyi kaybetme, o andan itibaren yeni kira geçerli olsun." Yeni `recurring_expenses` (kalem) + `recurring_expense_amounts` (aylık tutar sürümleri) - `tuition_rates` kalıbı, ay bazlı: yeni tutar seçilen ayın 1'inden geçerli, yürürlükteki sürüm önceki ayın son günü kapanır, geçmiş aylar değişmez. Yürürlükteki tutarın başladığı aydan önceye değişiklik 409. Aynı ayda ikinci değişiklik düzeltmedir: eski satır silinmez, `superseded_at` ile hesaptan çıkar. Kalem `POST .../end` ile bir ayın sonunda bitirilir, sonra ileri bir aydan yeniden başlatılabilir. DB: kalem başına tek açık tutar (`WHERE effective_until IS NULL AND superseded_at IS NULL`), kalemde xmin. Tek seferlik giderler (`expenses`) aynen kalır; ekrandaki her toplam ikisinin birleşimidir, gelecek aylar "planlanan" gösterilir ve net sonuca yalnızca bugüne kadarki gider girer. |
